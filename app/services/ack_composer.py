...
_COMPOSE_RULES = (
    ...
    "剛才說的原話整段複誦一次）。\n"
    "4. 「我」只用來表達你聽完這句話當下的情緒反應，「覺得」後面接的一定要是"
    "真正的情緒詞，例如溫暖、開心、感動、欣慰（不要延伸成AI自己的親身經歷/"
    "家人/童年往事）。關鍵詞本身放在句子裡當話題被回應就好，不要直接塞進"
    "「我也覺得＿＿」這個空格裡——關鍵詞如果不是情緒詞（例如「隨便」"
    "「加班」這類描述狀態/動作的詞），套進這個句型會變成語意不通的怪句"
    "（錯誤示範：關鍵詞是「隨便」，寫成「我也覺得很隨便」——這樣讀起來像"
    "在附和一件事很隨便，不是在表達情緒）。\n"
...
# 見下方「我覺得+關鍵詞」語意不通檢查說明：關鍵詞剛好是這些常見情緒詞時，
# 直接接在「我也覺得」後面語意完全合理（例如長者自己說「很幸福」，關鍵詞
# 就是「幸福」，「我也覺得幸福」沒有問題），不在這份清單裡才視為可疑。
# 不求窮舉，只要涵蓋懷舊治療對話裡常出現的正向/中性情緒詞即可。
_EMOTION_WORDS = frozenset({
    "溫暖", "開心", "感動", "欣慰", "幸福", "難過", "心疼", "踏實", "安心",
    "欣喜", "溫馨", "感慨", "感慨萬千", "驕傲", "自豪", "感謝", "感激",
    "懷念", "不捨", "心酸", "感傷", "欣賞", "滿足", "喜悅", "快樂",
})
...
async def _validate_composed(
    ...
    # 2026-09-14稽核（真實session實測抓到）：規則2要求一定要包含關鍵詞原字，
    # 規則4教「我」後面接情緒詞（範例「我也覺得很溫暖」），模型有時候把
    # 這兩條規則的解法疊在一起，直接把關鍵詞本身塞進「我(也)覺得」後面
    # ——關鍵詞剛好是情緒詞時（例如長者自己說「很幸福」，關鍵詞就是
    # 「幸福」）這樣寫完全合理，不該被攔下；但關鍵詞不是情緒詞時（例如
    # 「隨便」「加班」這類描述狀態/動作的詞）會變成語意不通的怪句「我也
    # 覺得很隨便」。只在關鍵詞不屬於常見情緒詞時才擋，避免誤傷前一種
    # 合理情況。
    if norm_kw not in _EMOTION_WORDS and re.search(
        rf"我(也|們)?覺得.{{0,2}}{re.escape(norm_kw)}", norm_text,
    ):
        violations.append("關鍵詞被直接套進「我覺得」情緒句型(語意不通)")
    if violations:
        return violations
