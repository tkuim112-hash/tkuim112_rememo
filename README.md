...
| 語音合成（主要路徑） | edge-tts（雲端） | Microsoft edge-tts（`zh-TW-HsiaoYuNeural` 台灣女聲）即時生成，見 [services/tts.py](app/services/tts.py) `synthesize_edge` |
| M3 語音合成（legacy） | `tts` | 本地 BlueMagpie-TTS（RTX 50 系列 / Blackwell 專用建置），服務與呼叫點仍保留，但已非主要路徑，見 [bluemagpie/](bluemagpie/) |
| M1 語音辨識 | `kinect-svc` | faster-whisper-server，載入中文微調 Whisper checkpoint `XA9/Belle-faster-whisper-large-v3-zh-punct` |
...
- 支援 NVIDIA GPU 的主機（`ollama`、`kinect-svc` 皆需要 GPU；`tts` 為 legacy 本地服務，仍需 GPU 但非必要啟動項目）
