...
# 2026-09-14 稽核：暫時改回 device="cuda"（09-14 曾經改成 cpu 讓 VRAM 給
# ollama，但懷疑 CPU 版本跑出來的 AU 數值跟 GPU 版有系統性落差——過去所有
# 門檻/基準都是拿 GPU 產出的數字校準的，貿然換成 CPU 會讓這些門檻對不準）。
# 先換回 GPU 排除這個變因，確認 AU 讀數恢復正常後再決定要不要處理 VRAM
# 排擠問題（例如限制 ollama context/模型大小，而不是犧牲這裡的判斷品質）。
